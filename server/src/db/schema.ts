...
export const orders = mysqlTable('orders', {
  ...
  paymentStatus: mysqlEnum('payment_status', ['pending', 'authorized', 'paid', 'verified', 'failed', 'refunded', 'partially_refunded']).notNull().default('pending'),
...
export const quotations = mysqlTable('quotations', {
  ...
  parentIdx: index('quotations_parent_idx').on(table.parentQuotationId)
}));
...
export const wholesaleOrders = mysqlTable('wholesale_orders', {
  ...
  quotationIdx: index('ws_orders_quotation_idx').on(table.quotationId),
  inquiryIdx: index('ws_orders_inquiry_idx').on(table.inquiryId),
  statusIdx: index('ws_orders_status_idx').on(table.status)
}));

export const wholesaleOrderItems = mysqlTable('wholesale_order_items', {
  id: serial('id').primaryKey(),
  wholesaleOrderId: int('wholesale_order_id').notNull(),
  productVariantId: int('product_variant_id'),
  productName: varchar('product_name', { length: 255 }).notNull(),
  weightLabel: varchar('weight_label', { length: 128 }),
  quantity: decimal('quantity', { precision: 12, scale: 3 }).notNull(),
  ...
  orderIdx: index('ws_order_items_order_idx').on(table.wholesaleOrderId)
}));

export const wholesaleInvoices = mysqlTable('wholesale_invoices', {
  id: serial('id').primaryKey(),
  ...
  orderIdx: index('ws_invoices_order_idx').on(table.wholesaleOrderId),
  statusIdx: index('ws_invoices_status_idx').on(table.status)
}));

export const wholesaleActivityLog = mysqlTable('wholesale_activity_log', {
  id: serial('id').primaryKey(),
  entityType: mysqlEnum('entity_type', ['inquiry', 'quotation', 'order', 'invoice']).notNull(),
  ...
  entityIdx: index('ws_activity_entity_idx').on(table.entityType, table.entityId),
  createdIdx: index('ws_activity_created_idx').on(table.createdAt)
}));
