...
function useSEO(title: string, description: string) {
 useEffect(() => {
 document.title = `${title} | Lepakshi Spices Premium Spices`;
...
function Header() {
 ...
 <p className="text-xs text-[#7A7064]">{formatINR(p.price)} · {p.weight}</p>
 </div>
 </button>
 ))}
 </motion.div>
...
function ProductPage({ product }: { product: any }) {
 ...
 <>
 <span className="text-xl text-[#7A7064] line-through">{formatINR(product.originalPrice)}</span>
 ...
 const isWholesaleTab = weightTab === "wholesale";
 const optStock = opt.id ? (product.variants?.find((v: any) => v.id === opt.id)?.stock ?? 0) : 0;
 const optOOS = !isWholesaleTab && optStock <= 0;
 return (
 <button
 key={opt.label}
 type="button"
 ...
 <span className="block text-[10px] mt-0.5 opacity-80">{formatINR(opt.price)}</span>
 {!isWholesaleTab && (optOOS ? <span className="block text-[9px] mt-0.5 text-red-400">Out of Stock</span> : optStock <= 10 ? <span className="block text-[9px] mt-0.5 text-amber-500">Only {optStock} left</span> : null)}
 </button>
 );
 })}
 </div>
 ...
 {[
 { icon: Truck, label: "Free Delivery", sub: `on orders ${formatINR(499)}` },
 ...
 <p className="text-xs text-[#7A7064]">{selectedWeightOpt.label.split(" ")[0]} · <strong className="text-[#1A1714]">{formatINR(selectedWeightOpt.price)}</strong></p>
 </div>
 </div>
 <div className="flex items-center gap-2">
...
function CartPage() {
 ...
 <p className="font-bold text-[#1A1714]">{formatINR(item.price * item.quantity)}</p>
 </div>
 {isOverStock && (
 ...
 <span className="font-medium text-[#2A4A3C]">-{formatINR(discount)}</span>
 </div>
 )}
 <div className="flex justify-between text-sm">
 ...
 <span className="font-bold text-xl text-[#1A1714]">{formatINR(total)}</span>
 </div>
 </div>
...
function WholesalePage() {
 ...
 const response = await fetch("/spices_logo.png");
 ...
 const drawPdfHeader = (doc: any, pageTitle: string, logoDataUrl: string, pageNumber: number, totalPages: number) => {
 doc.setFillColor(42, 74, 60);
 doc.rect(0, 0, 210, 24, "F");
 if (logoDataUrl) doc.addImage(logoDataUrl, "PNG", 14, 4, 16, 16);
 doc.setFont("Helvetica", "bold");
 doc.setFontSize(14);
 doc.setTextColor(255, 255, 255);
 doc.text("Lepakshi Spices", 34, 12);
 doc.setFont("Helvetica", "normal");
 doc.setFontSize(8.5);
 doc.text("Wholesale & Bulk Division", 34, 18);
 doc.setFont("Helvetica", "bold");
 doc.setFontSize(12);
 doc.text(pageTitle, 188, 12, { align: "right" });
 doc.setFont("Helvetica", "normal");
 doc.setFontSize(8);
 doc.text(`Page ${pageNumber} of ${totalPages}`, 188, 19, { align: "right" });
 };

const triggerPDFDownload = async () => {
 try {
  const [liveProducts, logoDataUrl] = await Promise.all([
 wholesaleCatalogueApi.list(),
 ...
 const selectedProductName = (formData.productInterest || "").trim().toLowerCase();
 const selectedVolume = (formData.volume || "").trim();
 const selectedVolumeMatch = selectedVolume.match(/(\d+(?:\.\d+)?)\s*kg/i);
 const selectedVolumeKey = selectedVolumeMatch ? `${selectedVolumeMatch[1]}kg` : selectedVolume.toLowerCase();
 const selectedProducts = selectedProductName
  ? (liveProducts || []).filter((p: any) => String(p.name || "").trim().toLowerCase() === selectedProductName)
  : (liveProducts || []);
 const catalogRows = selectedProducts.map((p: any) => {
  const allPacks = (p.packaging || []).filter((pack: any) => pack && pack.label);
  const selectedPack = allPacks.find((pack: any) => {
   const label = String(pack.label || "").trim().toLowerCase();
   return label === selectedVolume.toLowerCase() || label === selectedVolumeKey || label.includes(selectedVolumeKey);
  }) || allPacks.find((pack: any) => /25\s*kg/i.test(String(pack.label || ""))) || allPacks[0];
  return {
   name: p.name || "Unnamed product",
   category: p.category || "Spice",
   unit: selectedPack?.label || selectedVolume || p.weight || "1000g",
   price: Number(selectedPack?.price ?? p.prices?.p1000 ?? p.price ?? 0),
  };
 }).filter((row: any) => row.unit);

 doc.setFillColor(250, 248, 243);
 doc.rect(0, 0, 210, 297, "F");
 drawPdfHeader(doc, "Catalogue", logoDataUrl, 1, 2);
 doc.setFont("Helvetica", "bold");
 doc.setFontSize(20);
 doc.setTextColor(26, 23, 20);
 doc.text("Wholesale Price Catalogue", 20, 56);
 ...
 doc.text("Prepared for:", 20, 74);
 doc.setFillColor(255, 255, 255);
 doc.setDrawColor(26, 23, 20, 0.08);
 doc.roundedRect(20, 79, 170, 114, 4, 4, "FD");
 let cardY = 90;
 doc.setFont("Helvetica", "bold");
 doc.setFontSize(15);
 ...
 doc.text(pdfTemplate.watermark, 30, 240, null, 45);
 }

 doc.addPage();
 drawPdfHeader(doc, "Catalogue", logoDataUrl, 2, 2);
 doc.setFont("Helvetica", "bold");
 doc.setFontSize(17);
 doc.setTextColor(26, 23, 20);
 doc.text("Wholesale Price Catalogue", 20, 38);
 doc.setFont("Helvetica", "normal");
 doc.setFontSize(9.5);
 doc.setTextColor(122, 112, 100);
 doc.text("Live catalogue data pulled from the current product database at download time.", 20, 45);
 ...
 const splitText = (value: string, width: number) => doc.splitTextToSize(String(value || ""), width);
 const rows = catalogRows.length ? catalogRows : [{ name: formData.productInterest || "Selected Product", category: "Spice", unit: formData.volume || "Selected Variant", price: 0 }];

 const drawHeader = () => {
  doc.setFillColor(primary[0], primary[1], primary[2]);
  doc.roundedRect(pageLeft, headerY, pageWidth, 10, 2, 2, "F");
  doc.setFont("Helvetica", "bold");
  doc.setFontSize(9);
  doc.setTextColor(255, 255, 255);
  doc.text("Product Name", col.name.x, headerY + 6.2);
  doc.text("Category", col.category.x, headerY + 6.2);
  doc.text("Unit / Weight", col.unit.x, headerY + 6.2);
  doc.text("Wholesale Price", col.price.x + col.price.w, headerY + 6.2, { align: "right" });
 };
 ...
 rows.forEach((row: any) => {
  const nameLines = splitText(row.name, col.name.w);
  const categoryLines = splitText(row.category, col.category.w);
  const unitLines = splitText(row.unit, col.unit.w);
  const priceText = formatINR(row.price);
  const rowHeight = Math.max(nameLines.length, categoryLines.length, unitLines.length, 1) * 4.2 + 4.8;

  if (y + rowHeight > 270) {
   doc.addPage();
   drawPdfHeader(doc, "Catalogue", logoDataUrl, doc.getNumberOfPages(), doc.getNumberOfPages());
   doc.setFont("Helvetica", "bold");
   doc.setFontSize(17);
   doc.setTextColor(26, 23, 20);
   doc.text("Wholesale Price Catalogue", 20, 38);
  ...
  doc.setFillColor(255, 255, 255);
  doc.setDrawColor(26, 23, 20, 0.10);
  doc.roundedRect(pageLeft, y - 3.6, pageWidth, rowHeight, 1.8, 1.8, "FD");

  doc.setFont("Helvetica", "bold");
  doc.setFontSize(9.2);
  doc.setTextColor(26, 23, 20);
  doc.text(nameLines, col.name.x, y + 2.3);

  doc.setFont("Helvetica", "normal");
  doc.setTextColor(74, 70, 64);
  doc.text(categoryLines, col.category.x, y + 2.3);
  doc.text(unitLines, col.unit.x, y + 2.3);

  doc.setFont("Helvetica", "bold");
  doc.setFontSize(9.6);
  doc.setTextColor(42, 74, 60);
  doc.text(priceText, col.price.x + col.price.w, y + 2.3, { align: "right" });
 ...
 doc.setFontSize(8);
 doc.setTextColor(122, 112, 100);
 doc.text("Prepared using live database product data.", 20, 280);
